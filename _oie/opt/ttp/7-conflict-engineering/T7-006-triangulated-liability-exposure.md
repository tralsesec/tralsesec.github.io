---
layout: ttp
title: Triangulated Liability Exposure
ttp_id: T7-006
status: Active
---

## Procedure
1. **Node B Identification (The Delegation Trap):** Profile the hierarchy to identify a proxy (Node B) who holds "Execution Authority" for tasks Node A rubber-stamps. Ensure Node B is functionally essential so that Node A cannot easily replace them.
2. **Incubation:** Feed Node B mandates or constraints that increase their probability of localized, systemic error. Allow these errors to bake into their habitual output without alerting Node A.
3. **The Snap:** Trigger the disclosure via a "Helpful Third Party" inquiry (e.g., Audit, Compliance, or Quality Assurance) directed at Node A. Present these hardened errors not as new discoveries, but as persistent operational realities that should have been detected through standard oversight. Phrase the inquiry to explicitly question why these recurring blunders were not addressed, framing Node A's lack of intervention as either systemic negligence or active indifference toward Node B's performance.
4. **Forced Resolution:** Once the errors are undeniable, Node A is presented with a forced choice. They must either validate the flawed output, thereby absorbing the liability and inheriting the systemic problems, or sever the relationship with Node B, thereby triggering Triangulated Hostility.
